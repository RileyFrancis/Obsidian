...
***Proof:*** Let $h,h':X\rightarrow Y$ be homotopic and $k,k':Y \to Z$ be homotopic. Since each $h,h',k,k'$ are homotopic, they are each continuous, so their compositions $k \circ h$ and $k' \circ h'$ are continuous. Since $h\simeq h'$, there exists some continuous map $F:X\times I\rightarrow Y$ with $F(x,0)=h(x)$ and $F(x,1)=h'(x)$. Similarly, because $k\simeq k'$, there exists some continuous map $G:Y\times I\rightarrow Z$ with $G(x,0)=h(x)$ and $F(x,1)=h'(x)$
