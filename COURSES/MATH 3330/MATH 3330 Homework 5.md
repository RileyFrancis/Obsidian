...
***Proof:*** Let $\mathbb R^\omega$ have the product topology. Given sequences $(a_1,a_2,\ldots)$ and $(b_1,b_2,\ldots)$ of real numbers with $a_i>0$ for all $i$, we define the function $h:\mathbb R^\omega\rightarrow \mathbb R^\omega$ by the equation $$h\big((x_1,x_2,\ldots)\big)=(a_1x_1+b_1,a_2x_2+b_2,\ldots)$$Then, each $h_i=a_ix_i+b_i$ for all $i$. Since $h_i$ is a projection which is continuous in the product topology, and $a_ix_i+b_i$ itself is also continuous, $h$ itself must be continuous. Furthermore, because $a_i>0$, $h$ is bijective and we can define the continuous inverse:$$h^{-1}\big((y_1,y_2,\ldots)\big)=\left(\frac{y_1-b_1}{x_1},\frac{y_2-b_2}{x_2},\ldots\right)$$Therefore, $h$ is a homeomorphism of $\mathbb R^\omega$ to itself in the product topology.

Now let $\mathbb R^\omega$ have the box topology. Then, for $U=\prod_iU_i$$$h\left(U\right)=h\left(\prod_i U\right)=\prod_ia_iU_i+b_i$$Each $a_iU_i+b_i$ is open in $\mathbb R$, so $h(U)$ must also be open. We can also define the open inverse as $$h^{-1}\left(\prod_iV_i\right)=\prod_i\left(\frac{V_i-b_i}{a_i}\right)$$Therefore, $h$ is a homeomorphism of $\mathbb R^\omega$ to itself in the box topology.