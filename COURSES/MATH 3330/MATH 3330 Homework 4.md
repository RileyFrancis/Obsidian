...
> [!problem] Question 4
> Show that if $\mathcal A$ is a basis for a topology on $X$, then the topology generated by $A$ equals the intersection of all topologies on $X$ that contain $A$. Prove the same if $A$ is a subbasis.

***Proof:*** Let $\mathcal A$ be a basis for a topology on $X$ and let $\mathcal T_A$ be the topology generated by $\mathcal A$, so $\mathcal T_A=\bigcup_{U\subseteq A}U$. Also, let $\mathcal T$ be the intersection of all all topologies on $X$ that contain $A$. We must show that $\mathcal T_A = \mathcal T$ by double inclusion.

$\subseteq$ : Let $U\in \mathcal T_A$. 