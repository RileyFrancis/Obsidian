...
1. We can represent the standard 2-fold torus as $aba^{-1}b^{-1}cdc^{-1}d^{-1}$ (by gluing two standard tori together) in the shape of an octagon. Let $U$ be that octagon with a single point $p$ missing and let $V$ be a small disk around that point. Then $U\cap V$ is homotopy equivalent to $S^1$
