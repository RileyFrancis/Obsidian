------------------------------------------------------------
-- TEST SCRIPT FOR ELECTRONICS RETAILER DATABASE (SQLite)
...
INSERT INTO Inventory (productID, locationID, quantity) VALUES
    ...
    (103, 1, 7),    -- TVs in store (start above threshold)
...
------------------------------------------------------------
-- 11. BEFORE: check inventory for product 103 at store 1
------------------------------------------------------------
SELECT 'Inventory before in-store sale (product 103, location 1)' AS info;
SELECT * FROM Inventory WHERE productID = 103 AND locationID = 1;

------------------------------------------------------------
-- 12. Add Order Items (should decrement inventory via trigger)
--     Bob buys 5 TVs: 7 -> 2 (still above threshold, no reorder yet).
------------------------------------------------------------
INSERT INTO OrderItems (orderID, productID, quantity, salePrice)
VALUES (1000, 103, 5, 1499.99);

SELECT 'Inventory after order item insert (product 103, location 1)' AS info;
SELECT * FROM Inventory WHERE productID = 103 AND locationID = 1;

------------------------------------------------------------
-- 13. FORCE AUTO-REORDER BY DIRECTLY DROPPING INVENTORY BELOW 5
--     This guarantees the AFTER UPDATE trigger on Inventory fires,
--     even if the decrement trigger above had an issue.
------------------------------------------------------------
UPDATE Inventory
SET quantity = 3
WHERE productID = 103 AND locationID = 1;

SELECT 'Inventory after manual low-stock update (product 103, location 1)' AS info;
SELECT * FROM Inventory WHERE productID = 103 AND locationID = 1;

------------------------------------------------------------
-- 14. At this point, trg_Inventory_AutoReorder should have fired.
--     Check the Reorder table.
------------------------------------------------------------
SELECT 'Reorders after low inventory event' AS info;
SELECT * FROM Reorder;

------------------------------------------------------------
-- 15. Simulate receiving delivery for the most recent reorder
------------------------------------------------------------
INSERT INTO ReorderDelivery (deliveryID, reorderID, deliveryDate, quantityReceived)
VALUES
(
    5001,
    (SELECT reorderID FROM Reorder ORDER BY reorderID DESC LIMIT 1),
    '2025-12-05',
    50
);
...
------------------------------------------------------------
SELECT 'Inventory after delivery (product 103, location 1)' AS info;
SELECT * FROM Inventory WHERE productID = 103 AND locationID = 1;

------------------------------------------------------------
-- 16. Create ONLINE ORDER for Alice (billed to her account)
------------------------------------------------------------
INSERT INTO Orders (orderID, customerID, orderDate, orderType, locationID, accountID, paymentID)
VALUES
...
------------------------------------------------------------
-- Add online order items (will NOT affect store 1 inventory,
-- unless your triggers explicitly route online orders to a location).
------------------------------------------------------------
INSERT INTO OrderItems (orderID, productID, quantity, salePrice)
VALUES
    (2000, 101, 2, 379.99);
...
------------------------------------------------------------
-- 17. Add shipping for the online order
------------------------------------------------------------
...
------------------------------------------------------------
SELECT 'FINAL INVENTORY' AS info;
SELECT * FROM Inventory ORDER BY productID, locationID;

SELECT 'FINAL ORDERS' AS info;
SELECT * FROM Orders ORDER BY orderID;

SELECT 'FINAL ORDER ITEMS' AS info;
SELECT * FROM OrderItems ORDER BY orderID, productID;

SELECT 'FINAL SHIPPING' AS info;
SELECT * FROM Shipping ORDER BY shipmentID;

SELECT 'FINAL REORDERS' AS info;
SELECT * FROM Reorder ORDER BY reorderID;

SELECT 'FINAL REORDER DELIVERIES' AS info;
SELECT * FROM ReorderDelivery ORDER BY deliveryID;
