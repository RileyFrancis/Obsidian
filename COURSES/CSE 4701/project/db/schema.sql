...
--    (for orders that are tied to a location).
------------------------------------------------------------
CREATE TRIGGER trg_OrderItems_CheckInventory
BEFORE INSERT ON OrderItems
FOR EACH ROW
BEGIN
    -- Only enforce if the order has a location (in-person or
    -- if you choose to link online orders to a specific location).
    SELECT
        CASE
            WHEN
                (
                    IFNULL((
                        ...
                        WHERE o.orderID   = NEW.orderID
                          AND i.productID = NEW.productID
                    ), 0)
                ) < NEW.quantity
            THEN
                RAISE(ABORT, 'Insufficient inventory for this product at this location')
        END;
END;

------------------------------------------------------------
-- 2) Decrease inventory when an OrderItem is inserted
--    (again, only meaningful for orders tied to a location).
------------------------------------------------------------
...
------------------------------------------------------------
-- 3) Auto-reorder when inventory for a product at a location
--    drops below a threshold (per your choice: threshold=5,
--    reorder amount=50, per location).
--
--    We also avoid creating duplicate "open" reorders for the
--    same product+location pair by checking for existing
--    Pending/Ordered rows.
------------------------------------------------------------
...
BEGIN
    ...
    SELECT
        IFNULL((SELECT MAX(reorderID) FROM Reorder), 0) + 1,
        NEW.productID,
        NEW.locationID,
        p.manufacturerID,
        DATE('now'),
    ...
    WHERE p.productID = NEW.productID
      AND NOT EXISTS (
          ...
          WHERE r.productID  = NEW.productID
            AND r.locationID = NEW.locationID
            AND r.status IN ('Pending', 'Ordered')
      );
END;

------------------------------------------------------------
-- 4) When a ReorderDelivery arrives:
--    - Increase Inventory quantity at the appropriate location.
--    - Mark the Reorder as 'Received'.
------------------------------------------------------------
